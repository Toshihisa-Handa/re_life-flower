...
router.get('/', (req, res) => {
  let kensaku = req.query.kensaku;
    var sql = "SELECT S.name AS shopname, D.id, D.user_id, D.title, D.image, D.tag, D.text,ifnull(U.name, \'名無し\') AS name, DATE_FORMAT(D.created_at, \'%Y年%m月%d日 %k時%i分%s秒\') AS created_at FROM diary D LEFT OUTER JOIN user U ON D.user_id = U.user_id JOIN shop S ON D.user_id = S.user_id WHERE S.name LIKE '%"+kensaku+"%' ORDER BY D.created_at DESC"; // 変更
